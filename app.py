...
for idx, p in enumerate(st.session_state.peserta):
    nama_s = st.sidebar.text_input("Nama", key=f"peserta_nama_{idx}", label_visibility="collapsed", placeholder="isi nama")
    nip_s = st.sidebar.text_input("NIP", key=f"peserta_nip_{idx}", label_visibility="collapsed", placeholder="isi NIP")
    jabatan_s = st.sidebar.text_input("Jabatan", key=f"peserta_jab_{idx}", label_visibility="collapsed", placeholder="isi jabatan")
    if st.sidebar.button("Hapus", key=f"peserta_del_{idx}"):
...
with st.form("nota_dinas_form"):
    ...
    st.header("Tanda Tangan & Tembusan")

    submitted = st.form_submit_button("Generate Nota Dinas", use_container_width=True)
